...
const RECIPES_DATA_PATH := "res://data/crafting_recipes.json"
...
func get_max_craft_amount(recipe_id: String) -> int:
	...
	if ingredients.is_empty():
		return 0
	var max_amount: int = 999999
	for item_id_variant in ingredients.keys():
		var item_id: String = String(item_id_variant)
		var required_amount: int = maxi(int(ingredients[item_id_variant]), 1)
		max_amount = mini(max_amount, int(floor(float(_get_total_item_count(item_id)) / float(required_amount))))
	return max_amount


func can_craft(recipe_id: String, amount: int) -> bool:
	if amount <= 0:
		return false
	var recipe: Dictionary = get_recipe(recipe_id)
	if recipe.is_empty():
	...
	for item_id_variant in ingredients.keys():
		var item_id: String = String(item_id_variant)
		var required_amount: int = int(ingredients[item_id_variant]) * amount
		if _get_total_item_count(item_id) < required_amount:
			return false
	return true


func consume_ingredients(recipe_id: String, amount: int) -> bool:
